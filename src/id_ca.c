                              ...
                              byte *dest, size_t dest_len)
{
    ...
    while (op < dest_len) {
        if (ip + 2 > src_len) break;
        word ch = source[ip] | (source[ip + 1] << 8);
        ip += 2;
        word tag = ch >> 8;
        word count = ch & 0xFF;
        ...
        if (tag == NEARTAG || tag == FARTAG) {
            if (count == 0) {
                ...
                if (ip >= src_len) break;
                word val = ch | source[ip++];
                dest[op++] = val & 0xFF;
                if (op < dest_len) dest[op++] = (val >> 8) & 0xFF;
            } else if (tag == NEARTAG) {
                // near copy: 1-byte backward offset
                if (ip >= src_len) break;
                size_t cp = op - (size_t)source[ip++] * 2;
                while (count-- > 0 && op + 2 <= dest_len) {
                    dest[op++] = dest[cp++];
                    dest[op++] = dest[cp++];
                }
            } else {
                // far copy: 2-byte absolute offset
                if (ip + 2 > src_len) break;
                size_t cp = (size_t)(source[ip] | (source[ip + 1] << 8)) * 2;
                ip += 2;
                while (count-- > 0 && op + 2 <= dest_len && cp + 2 <= dest_len) {
                    dest[op++] = dest[cp++];
                    dest[op++] = dest[cp++];
                }
            }
...
{
    // In original this read from a far pointer / file.
    // Now a no-op placeholder.
    (void)dest;
    (void)length;
}
...
void CA_Startup(void)
{
    ...
    // --- Load VGAHEAD.WL6 (offset table for VGAGRAPH) ---
    {
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            grstarts[i] = (longword)b[0] | ((longword)b[1] << 8) | ((longword)b[2] << 16);
        }
        // Sentinel: one past the last entry for size calculation
        grstarts[count] = 0xFFFFFFFF;
        fclose(f);
    }
    ...
    // --- Load MAPHEAD.WL6 (RLEW tag + map offsets) ---
    {
        const char *paths[] = {
        ...
        // First 2 bytes are the RLEW tag (word), not longword
        word rlew_tag_word;
        if (fread(&rlew_tag_word, sizeof(word), 1, f) != 1) {
            fclose(f);
            Quit("CA_Startup: Failed to read RLEW tag");
        }
        ca_rlew_tag = (longword)rlew_tag_word;

        // Read map offsets and headers from GAMEMAPS
        for (i = 0; i < NUMMAPS; i++) {
            longword map_offset;
            if (fread(&map_offset, sizeof(longword), 1, f) != 1) {
                break;
            }
            if (map_offset == 0) {
                mapheaderseg[i] = NULL;
            ...
            // Allocate and read the map header (maptype struct)
            ...
            // We need to read the header from GAMEMAPS, not MAPHEAD.
            // The map_offset points into GAMEMAPS.WL6 where the header is.
            // Store the offset for later use in CA_CacheMap.
            // For now just zero-initialize; actual read happens in CA_CacheMap.
            memset(mapheaderseg[i], 0, sizeof(maptype));
        }
        fclose(f);
    }
...
void CA_CacheGrChunk(int chunk)
{
    ...
    if (offset == 0xFFFFFFFF) return; // sparse tile

    // Find next non-sparse chunk for length calculation
    longword next_offset = 0;
    for (int next = chunk + 1; next <= NUMCHUNKS; next++) {
        if (next == NUMCHUNKS) { next_offset = offset + 1; break; }
        next_offset = grstarts[next];
        if (next_offset != 0xFFFFFFFF) break;
    }

    longword total_length = next_offset - offset;
    if (total_length <= 4) return; // need at least the 4-byte header
    ...
    // Chunk format: first 4 bytes = expanded (decompressed) size (little-endian)
    // Remaining bytes = Huffman-compressed data
    longword expanded = (longword)raw_data[0]
                      | ((longword)raw_data[1] << 8)
                      | ((longword)raw_data[2] << 16)
                      | ((longword)raw_data[3] << 24);
    byte *huff_source = raw_data + 4;

    if (expanded == 0 || expanded > 65536) {
        free(raw_data);
        return;
    }
...
void CA_CacheMap(int mapnum)
{
    ...
    if (!ca_mapfile) return;

    mapon = mapnum;

    // Read map header from GAMEMAPS
    // We need to re-read the offset from MAPHEAD, then seek in GAMEMAPS.
    // For simplicity, re-open MAPHEAD to get the offset.
    {
        const char *paths[] = {
            "assets/wl6/MAPHEAD.WL6",
            "MAPHEAD.WL6",
        ...
        // Skip RLEW tag (2 bytes) + mapnum * 4 bytes of offsets before this one
        fseek(mf, 2 + mapnum * (long)sizeof(longword), SEEK_SET);
        longword map_offset;
        if (fread(&map_offset, sizeof(longword), 1, mf) != 1 || map_offset == 0) {
            fclose(mf);
            return;
        }
        fclose(mf);

        // Seek to map data in GAMEMAPS
        fseek(ca_mapfile, (long)map_offset, SEEK_SET);

        // Read maptype header
        if (!mapheaderseg[mapnum]) {
            mapheaderseg[mapnum] = (maptype *)malloc(sizeof(maptype));
        }
        if (!mapheaderseg[mapnum]) return;

        if (fread(mapheaderseg[mapnum], sizeof(maptype), 1, ca_mapfile) != 1) {
            return;
        }
        ...
        for (plane = 0; plane < MAPPLANES; plane++) {
            ...
            fseek(ca_mapfile, (long)plane_start, SEEK_SET);
