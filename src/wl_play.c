// WL_PLAY.C - Play loop, controls, actor list, music, palette shifts
// Ported from original WOLFSRC/WL_PLAY.C to C11/SDL3
//
// Changes from original:
//   - Removed #pragma hdrstop, #pragma warn directives
//   - Removed DOS-specific mouse/joystick polling (Mouse(MDelta), INL_GetJoyDelta)
//   - Removed far pointer qualifiers
//   - Replaced DOS timer busy-wait with SD_Poll/TimeCount based timing
...
void PollJoystickButtons (void)
{
	...
	buttons = IN_JoyButtons ();	// SDL3: reads game controller buttons
...
void PollMouseMove (void)
{
	...
	// SDL3: read mouse delta via IN_ReadCursor
	IN_ReadCursor(&ci);
	mousexmove = ci.x;
	mouseymove = ci.y;
...
void PollJoystickMove (void)
{
	...
	// SDL3: read joystick axis via cursor interface
	CursorInfo ci;
	IN_ReadCursor(&ci);
	joyx = ci.x;
	joyy = ci.y;
...
void StopMusic(void)
{
	...
	for (i = 0;i < LASTMUSIC;i++)
		...
		{
			memptr p = (memptr)audiosegs[STARTMUSIC + i];
			MM_SetPurge(&p,3);
			MM_SetLock(&p,false);
		}
}
...
//
	...
	if (!ob->ticcount)
	{
		...
		actorat[ob->tilex][ob->tiley] = (uintptr_t)ob;
		return;
	}
