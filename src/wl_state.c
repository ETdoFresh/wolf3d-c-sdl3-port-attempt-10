// wl_state.c - Actor state management, pathfinding, combat (SDL3 port)
// Ported from original WOLFSRC/WL_STATE.C
...
void SpawnNewObj (unsigned tilex, unsigned tiley, statetype *state)
{
	...
	actorat[tilex][tiley] = (uintptr_t)new;
	new->areanumber =
...
{                                                   \
	if ((x)>=(unsigned)MAPSIZE||(y)>=(unsigned)MAPSIZE)\
		return false;                               \
	temp=actorat[x][y];                             \
	if (temp)                                       \
	{                                               \
