// wl_text.c - Text display, help screens, end articles (SDL3 port)
// Ported from original WOLFSRC/WL_TEXT.C
...
void PageLayout(boolean shownumber)
{
    ...
    {
        strcpy(strbuf, "pg ");
        itoa(pagenum, str2buf, 10);
        strcat(strbuf, str2buf);
        strcat(strbuf, " of ");
        py = 183;
        px = 213;
        itoa(numpages, str2buf, 10);
        strcat(strbuf, str2buf);
        fontcolor = 0x4f;

        VWB_DrawPropString(strbuf);
    }
