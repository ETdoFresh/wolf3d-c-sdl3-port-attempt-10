// wl_game.c - Game loop and level setup (SDL3 port)
...
void SetupGameLevel(void)
{
    ...
    if (!mapheaderseg[mapnum]) {
        Quit("SetupGameLevel: map header not loaded");
    }
    ...
    //
    // build farmapylookup table
    //
    if (!farmapylookup)
        farmapylookup = (word *)malloc(MAPSIZE * sizeof(word));
    for (i = 0; i < MAPSIZE; i++)
        farmapylookup[i] = mapwidth * i;
...
void FinishDemoRecord(void)
{
    ...
    *(unsigned *)demoptr = length;
    ...
    if (US_LineInput(PrintX, PrintY, str, NULL, true, 2, 0))
    {
        level = atoi(str);
        if (level >= 0 && level <= 9)
        {
            demoname[4] = '0' + level;
            CA_WriteFile(demoname, (byte *)demobuffer, length);
        }
    }
...
void PlayDemo(int demonumber)
{
    ...
    // debug: load chunk from embedded data
