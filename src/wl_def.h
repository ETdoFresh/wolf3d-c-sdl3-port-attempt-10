// WL_DEF.H - Master game definition header (ported from original WL_DEF.H)
...
// SPANISH/FOREIGN not needed for WL6 English build
#include "foreign.h"
...
#define MAPSPOT(x,y,plane)		(*(mapsegs[plane] + (mapwidth * (y)) + (x)))
...
typedef struct	statestruct
{
	...
	int		tictime;
	void	(*think)(struct objstruct *ob), (*action)(struct objstruct *ob);
...
// actorat stores either small integers (wall/door tiles) or objtype* pointers.
// Use this macro to extract a real pointer (NULL if it's just a tile value).
#define ACTOR_AT(x,y)		((actorat[x][y] > 255) ? (objtype *)actorat[x][y] : (objtype *)0)
...
extern	byte		*postpage;
extern	unsigned	posttex;
...
// t_compshape: sprite data format loaded from the page manager.
// On-disk fields are 16-bit (DOS unsigned = 16-bit).
typedef struct
{
